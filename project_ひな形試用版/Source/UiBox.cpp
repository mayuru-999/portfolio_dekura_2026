...
void UiBox::Draw()
{
	...
	int labelW = GetDrawStringWidthToHandle(m_text.c_str(), m_text.size(), m_font);
	int labelX = m_position.x + (m_size.x - labelW) / 2;

	DrawBox(topL.x, topL.y, bottomR.x, bottomR.y, m_color, TRUE);
	DrawStringFToHandle(topL.x, topL.y, m_text.c_str(), m_fontColor, m_font);
}
